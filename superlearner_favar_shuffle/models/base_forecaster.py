...
#
# NOTE: The FAVAR-Net does NOT extend this class (it uses flat
# inputs, not sequences). Kept here for reference/compatibility.
# ================================================================
    ...
    def forecast_features(self, X_current: np.ndarray, X_seq: np.ndarray) -> np.ndarray:
        ...
        for i in range(self.n_vars):
            for s in range(self.forecast_steps):
                features[:, idx] = forecasts[:, s, i]
                features[:, idx + 1] = forecasts[:, s, i] - X_current[:, i]
                idx += 2
